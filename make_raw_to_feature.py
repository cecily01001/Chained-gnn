...
# def load_data(args,train,text,label):
#
...
def load_model(args):
    ...
    model = TextCNN(args)
    model.load_state_dict(torch.load('CNNModel/model_dir/best_steps_2000.pt'))
    model.eval()
    return model
...
def get_text_feature(train,text,label):
    text_list=[]
    args=load_args()
    train_iter = load_data(args,train,text,label)
    ...
    for batch in train_iter:
        ...
        for i in logits:
            text_list.append(i.detach().numpy().tolist())
    # print(text_list)
    return text_list
def transform_pcap(path,label_dic,datakind):
    rows=[]
    direcs=[]
    # 数据集定义
    examples = []
    ...
    f = open('CNNModel/singletrain.csv', 'w', encoding='utf-8', newline="")
    tsv_w = csv.writer(f)
    tsv_w.writerow(['label', 'text'])
    save_dir=os.path.join('Dataset/Processed_data/',datakind)
    ...
    for i, packet in enumerate(packets_list):
        ...
        if arr is not None:
            ...
            examples.append(data.Example.fromlist([str(raw(packet)), app_label], fields))
...
def main(kind):
    if kind == 'app':
        ...
        make_feature("Dataset/Splite_Session/Entropy/train", "Dataset/Splite_Session/Entropy/valid",
